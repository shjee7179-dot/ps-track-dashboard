...
`src/lib/lms/readonly-adapter.ts` provides the current `LMS_PROVIDER=none|mock-view|readonly-db` adapter. `mock-view` uses only synthetic IDs and sanitized sample records so screens can be developed before real LMS IDs are available.

`readonly-db` is implemented against this contract only. It expects the LMS operations team to provide readonly views with the exact contract columns below, or to set custom view names through environment variables. It must not be pointed at original LMS tables directly.
...
```text
LMS_PROVIDER=none | mock-view | readonly-db
```
...
| `readonly-db` | LMS 운영팀이 제공한 readonly view를 별도 read connection으로 조회 |

```text
LMS_PROVIDER=readonly-db
LMS_DATABASE_URL=postgres://readonly_user:password@lms-readonly-host:5432/lms_database
LMS_POSTGRES_SSL=true | false
LMS_CONTENT_CATALOG_VIEW=lms_content_catalog_view
LMS_LEARNING_RECORD_VIEW=lms_learning_record_view
LMS_READONLY_QUERY_LIMIT=500
```

`LMS_CONTENT_CATALOG_VIEW` and `LMS_LEARNING_RECORD_VIEW` may include a schema prefix such as `readonly.lms_content_catalog_view`. Only normal PostgreSQL identifier segments are accepted.
...
실제 LMS DBMS, view 명세, 사용자 식별자 매핑, 상태 코드 목록을 받기 전에는 운영 DB 접속을 활성화하지 않는다.

현재 코드는 contract 기반 `readonly-db` adapter까지 준비하지만, 운영 적용은 LMS 운영팀이 제공한 readonly view가 위 컬럼 계약을 만족하는지 확인한 뒤 진행한다.
