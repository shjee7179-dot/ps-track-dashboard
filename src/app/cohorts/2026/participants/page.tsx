  ...
  return (
    <AppShell title="참여자 관리" eyebrow="Cohort / Participants">
      <div className="mb-6 grid gap-4 sm:grid-cols-3">
        ...
        <Stat label="운영자" value={`${users.filter((user) => user.defaultRole === "operator").length}명`} />
        <Stat label="멘토" value={`${users.filter((user) => user.defaultRole === "mentor").length}명`} />
      </div>
      <Card title="참여자 목록" subtitle="학생 개인 기준 여정으로 바로 이동">
        <div className="space-y-3">
          {participants.map((participant) => {
            const summary = getJourneySummary(participant.id);
            return (
              <Link
                key={participant.id}
