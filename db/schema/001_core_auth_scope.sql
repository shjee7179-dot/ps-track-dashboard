-- Core auth/scope schema draft
-- Purpose: fix the first real table shape before Supabase Auth and DB-backed
...
comment on table public.users is
  'Application domain users. Supabase Auth identity is linked through auth_user_id.';

comment on column public.users.auth_user_id is
  'Nullable until the user has a Supabase Auth account. References auth.users.id.';

comment on column public.users.external_subject is
  'Future integration subject for LMS/Keycloak or another identity provider.';
